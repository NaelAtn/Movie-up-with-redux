...
const movieReducer = (state = initialeState , {type,payload}) => {
    switch(type) {
        case ADD_MOVIE :
            return {...state , listMovies : [...state.listMovies,payload]}
        case DELETE_MOVIE :
            return {...state , listMovies :state.listMovies.filter(el => el.id !== payload)}   
        default :
            return state ; 
    }
}
